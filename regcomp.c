                               ...
                               const regnode_charclass* const node)
{
    ...
    const U8 flags = (OP(node) == ANYOFH || OP(node) == ANYOFHb)
                      ? 0
                      : ANYOF_FLAGS(node);
                ...
                const regnode_charclass *and_with)
{
    ...
    SV* anded_cp_list;
    U8  and_with_flags = (OP(and_with) == ANYOFH || OP(and_with) == ANYOFHb)
                          ? 0
               ...
               const regnode_charclass *or_with)
{
    ...
    SV* ored_cp_list;
    U8 ored_flags;
    U8  or_with_flags = (OP(or_with) == ANYOFH || OP(or_with) == ANYOFHb)
                         ? 0
                         : ANYOF_FLAGS(or_with);
...
#define isSINGLE_BIT_SET(n) isPOWER_OF_2(n)
            ...
            &&   anyof_flags == 0)
        {
            ...
            if (highest_cp <= IV_MAX) {
                ...
                /* For start bytes C2-DF, we could have another regnode type
                 * that uses the lower 5 bits + C2 to give the lowest, and the
                 * upper 3 bits to give the maximum, so we could exclude too
                 * high start bytes too */
...
Perl_regprop(pTHX_ const regexp *prog, SV *sv, const regnode *o, const regmatch_info *reginfo, const RExC_state_t *pRExC_state)
{
	...
	const U8 flags = (OP(o) == ANYOFH || OP(o) == ANYOFHb)
                          ? 0
                          : ANYOF_FLAGS(o);
	...
	Perl_sv_catpvf(aTHX_ sv, "%s]", PL_colors[1]);
        ...
        if (OP(o) == ANYOFH || OP(o) == ANYOFHb) {
            const char * phrase = (OP(o) == ANYOFH) ? "Lowest first" : "First";
            Perl_sv_catpvf(aTHX_ sv, " (%s UTF-8 byte=\\x%02x)",
                                     phrase, FLAGS(o));
        }
